...
class RVSA_MTP_branches(nn.Module):
    ...
    def init_weights(self):
        ...
        if isinstance(pretrained, str):
            ...
            state_dict = {k: v for k, v in state_dict.items() if 'decoder' not in k and 'mask_token' not in k and 'cls_token' not in k}

            msg = self.load_state_dict(state_dict, False)
            #print("Loaded keys:", [k for k in state_dict.keys() if 'patch_embed' in k or 'blocks' in k])
            #print("Backbone weights in model:", [k for k in self.state_dict().keys() if 'patch_embed' in k or 'blocks' in k])
